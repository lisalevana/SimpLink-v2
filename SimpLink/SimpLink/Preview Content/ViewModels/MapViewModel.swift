...
class MapViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    @Published var currentLocationResults: [MKMapItem] = []
    @Published var destinationResults: [MKMapItem] = []
    ...
    override init() {
        ...
        // Debounce searchFrom and searchTo
        $searchFrom
            .debounce(for: .milliseconds(400), scheduler: RunLoop.main)
            .sink { [weak self] query in
                self?.searchSuggestions(for: query, isFrom: true)
            }
        ...
        $searchTo
            .debounce(for: .milliseconds(400), scheduler: RunLoop.main)
            .sink { [weak self] query in
                self?.searchSuggestions(for: query, isFrom: false)
            }
            .store(in: &cancellables)
    }
        ...
        for route in allRoutes {
            if let fromIndex = route.stops.firstIndex(of: from),
               ...
               fromIndex < toIndex {
                ...
                let segmentStops = Array(route.stops[fromIndex...toIndex])
                let segmentRoute = BusRoute(
                    id: route.id,
                    name: route.name,
                    stops: segmentStops,
                    color: route.color,
                ...
                suggestions.append(RouteSuggestion(
                    routes: [segmentRoute], // Use the sliced segment
                ...
                for transferStop in transferStops {
                    if let fromIndex = fromRoute.stops.firstIndex(of: from),
                       ...
                       fromIndex < transferIndex1 && transferIndex2 < toIndex {
                        ...
                        let fromSegmentStops = Array(fromRoute.stops[fromIndex...transferIndex1])
                        let toSegmentStops = Array(toRoute.stops[transferIndex2...toIndex])
                        let fromSegmentRoute = BusRoute(
                            id: fromRoute.id,
                            name: fromRoute.name,
                            stops: fromSegmentStops,
                            color: fromRoute.color,
                            departureTimes: fromRoute.departureTimes
                        )
                        let toSegmentRoute = BusRoute(
                            id: toRoute.id,
                            name: toRoute.name,
                            stops: toSegmentStops,
                            color: toRoute.color,
                            departureTimes: toRoute.departureTimes
                        )
                        suggestions.append(RouteSuggestion(
                            routes: [fromSegmentRoute, toSegmentRoute], // Use the sliced segments
                            totalDuration: duration,
                            transitCount: 1,
                            departureTime: generateNextDepartureTime()
                        ))
    ...
    private static let tangerangSpecificLocalities = [
        "Serpong", "Ciputat", "Pamulang", "Pondok Aren", "Karawaci", "Ciledug", "BSD", "Bintaro Sektor"
    ]

    private var tangerangSearchRegion: MKCoordinateRegion {
        MKCoordinateRegion(
            center: CLLocationCoordinate2D(latitude: -6.202394, longitude: 106.652710), // BSD City center
            span: MKCoordinateSpan(latitudeDelta: 0.4, longitudeDelta: 0.4)
        )
    }

    private func isWithinTangerang(_ location: CLLocationCoordinate2D) -> Bool {
        let tangerangRegion = CLCircularRegion(
            center: CLLocationCoordinate2D(latitude: -6.302176166973364, longitude:  106.65266850473363),
            radius: 30000, // 30 km
            identifier: "Tangerang"
        )
        return tangerangRegion.contains(location)
    }
    ...
    func searchSuggestions(for query: String, isFrom: Bool) {
        ...
        let filteredBusStops = bsdBusStops.filter {
            $0.name.localizedCaseInsensitiveContains(query) && isWithinTangerang($0.location)
        }
        ...
        DispatchQueue.main.async {
            let filteredLocations = self.locationSearchResults.map {
                BusStop(
                    id: UUID().uuidString,
                    name: "\($0.title), \($0.subtitle)",
                    location: CLLocationCoordinate2D(), // coordinate unknown until selected
                    isBusStop: false
                )
            }
            if isFrom {
                self.fromSuggestions = filteredBusStops + filteredLocations
            } else {
                self.toSuggestions = filteredBusStops + filteredLocations
            }
        }    }
    ...
    private func searchLocation(query: String, isForCurrentLocation: Bool) {
        ...
        Task {
            do {
                ...
                let filteredResults = response.mapItems.filter { isWithinTangerang($0.placemark.coordinate) }
                await MainActor.run {
                    if isForCurrentLocation {
                        self.currentLocationResults = filteredResults
                    } else {
                        self.destinationResults = filteredResults
                    }
                }
            } catch {
                print("Search failed for query '\(query)': \(error.localizedDescription)")
                await MainActor.run {
                    if isForCurrentLocation { self.currentLocationResults = [] } else { self.destinationResults = [] }
                }
            }
        }
    }
...
extension MapViewModel: MKLocalSearchCompleterDelegate {
    func completerDidUpdateResults(_ completer: MKLocalSearchCompleter) {
        self.locationSearchResults = completer.results
        DispatchQueue.main.async {
            self.updateSuggestions()
        }
    }

    // 3. Update searchSuggestions to use completions directly
    private func updateSuggestions() {
        let filteredBusStops = bsdBusStops.filter {
            $0.name.localizedCaseInsensitiveContains(searchFrom) && isWithinTangerang($0.location)
        }
        let locationNames = locationSearchResults.map {
            BusStop(id: UUID().uuidString, name: "\($0.title), \($0.subtitle)", location: CLLocationCoordinate2D(), isBusStop: false)
        }
        if !searchFrom.isEmpty {
            fromSuggestions = filteredBusStops + locationNames
        }
        if !searchTo.isEmpty {
            toSuggestions = filteredBusStops + locationNames
        }
    }

    // 4. Add a method to resolve coordinates when a suggestion is selected
    func selectSuggestion(_ completion: MKLocalSearchCompletion, isFrom: Bool) {
        getCoordinates(for: completion) { coordinate in
            guard let coordinate else { return }
            let stop = BusStop(
                id: UUID().uuidString,
                name: "\(completion.title), \(completion.subtitle)",
                location: coordinate,
                isBusStop: false
            )
            DispatchQueue.main.async {
                if isFrom {
                    self.selectedFrom = stop
                    self.searchFrom = stop.name
                } else {
                    self.selectedTo = stop
                    self.searchTo = stop.name
                }
            }
        }
    }
